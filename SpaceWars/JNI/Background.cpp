...
Background::Background(android_app* pApplication,
	TimeManager& pTimeManager, GraphicsManager& pGraphicsManager, Resource& pTextureResource) :
...
status Background::load()
{
	...
	vertexBuffer->x = mGraphicsManager.getRenderWidth() / 6;
	vertexBuffer->y = mGraphicsManager.getRenderHeight();
	vertexBuffer->z = 1.0f;
	// Loads the vertex buffer into OpenGL.
	mVertexBuffer = mGraphicsManager.loadVertexBuffer(vertexBuffer, sizeof(Vertex));
	delete vertexBuffer;
	if (mVertexBuffer == 0) 
		goto ERROR;
	// Loads the texture.
