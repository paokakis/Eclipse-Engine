...
/*Here, we configure the data source as a MIME source so that the file type gets
...
The data sink (that is, the destination channel) is configured with the OutputMix
object created in the first part of this chapter while initializing the OpenSL ES engine
(and refers to the default audio output, that is, speakers or headset):*/
status SoundManager::playBGM(Resource& pResource) 
{
	...
	its end (SL_TIME_UNKNOWN), and then start playing (SetPlayState() with SL_PLAYSTATE_PLAYING).*/
